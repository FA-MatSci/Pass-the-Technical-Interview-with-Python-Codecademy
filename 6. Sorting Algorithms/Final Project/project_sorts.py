...
def bubble_sort(arr, comparison_function):
  ...
  while not sorted:
    ...
    for idx in range(len(arr) - 1):
      if comparison_function(arr[idx], arr[idx + 1]):   #you have conditions defined in this function
        sorted = False
...
def quicksort(list, start, end, comparison_function):
  ...
  for i in range(start, end):
    if comparison_function(pivot_element, list[i]):     #here the condition is defined
